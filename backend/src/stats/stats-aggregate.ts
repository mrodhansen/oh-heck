...
export type StatsMode = 'users' | 'players';

export type StatsGameSnap = {
  id: string;
  name: string | null;
  status: GameStatus;
  createdAt: Date;
...
export function buildStats(games: readonly StatsGameSnap[], mode: StatsMode) {
  ...
  for (const game of games) {
    ...
    const seats = players.map((p) => seatForMode(p, mode));
...
function seatForMode(
  ...
  if (mode === 'users') return userSeat(p);
  return playerSeat(p);
}
